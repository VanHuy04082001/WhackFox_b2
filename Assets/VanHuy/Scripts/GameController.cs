...
namespace VanHuy
{
    public class GameController : MonoBehaviour
    {
        ...
        {
            Time.timeScale = 1;
            indexLv = PlayerPrefs.GetInt("IndexLv");
            LoadCoin();
            SpawnLevel();
        }
        void Start()
        ...
        public void SetEndTime()
        {
            if (score >= listTargetScores[indexLv])
            {
                ...
                MusicController.ins.SoundGame(MusicController.ins.audioClipWin);
            }
            else
            {
                logicGame.uiController.ShowPanelLose();
                MusicController.ins.SoundGame(MusicController.ins.audioClipLose);
            }
        }
    }
}
